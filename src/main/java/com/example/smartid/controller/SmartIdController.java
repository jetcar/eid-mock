package com.example.smartid.controller;

import com.example.smartid.dto.AuthenticationSessionRequest;
import com.example.smartid.dto.AuthenticationSessionResponse;
import com.example.smartid.dto.SessionStatusResponse;
import com.example.smartid.service.SessionStore;
...
public class SmartIdController {
    ...
    public ResponseEntity<AuthenticationSessionResponse> authenticateByPersonalCode(
            @Parameter(description = "Personal identification code with country prefix (e.g., PNOEE-38509076512)", required = true) @PathVariable String personalCode,
        ...
        String sessionId = sessionStore.createSession(actualPersonalCode, country, request.getHash(), givenName,
                surname);
    ...
    public ResponseEntity<SessionStatusResponse> getSessionStatus(
        ...
        // If session is running and timeout is specified, wait a bit
        if ("RUNNING".equals(response.getState()) && timeoutMs != null && timeoutMs > 0) {
            try {
                Thread.sleep(Math.min(timeoutMs, 3000)); // Wait max 3 seconds
                response = sessionStore.getSessionStatus(sessionId);
